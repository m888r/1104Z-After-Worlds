...
void AdaptivePurePursuit::loop()
{
	...
	path::Point robotPosition = {motioncontrol::currX, motioncontrol::currY};
	...
	QAngle currBearingAngle = std::atan2(sin(motioncontrol::currAngle.convert(radian)), cos(motioncontrol::currAngle.convert(radian))) * radian;
	...
	discoChassis.driveVector(direction * forwardPower, turnPower); // TODO CHASSIS MODEL IN CONSTRUCTOR INSTEAD OF HERE
}
...
void AdaptivePurePursuit::ramseteLoop()
{
	...
	QAngle currBearingAngle = std::atan2(std::sin(motioncontrol::currAngle.convert(radian)), std::cos(motioncontrol::currAngle.convert(radian))) * radian;
	...
	discoChassis.driveVector(direction * v_out, w_out);
} // namespace pathfollowing
...
void AdaptivePurePursuit::checkIsSettled()
{
	...
	double angleError = atan2(sin(finalAngleTarget - motioncontrol::currAngle.convert(okapi::radian)), cos(finalAngleTarget - motioncontrol::currAngle.convert(okapi::radian)));

	double finalDistance = sqrt(pow((point.x.convert(okapi::inch) - motioncontrol::currX.convert(okapi::inch)), 2) + pow((point.y.convert(okapi::inch) - motioncontrol::currY.convert(okapi::inch)), 2));
	if (timeout.convert(okapi::millisecond) > 0)
	{
		_isSettled = (distanceSettledUtil.isSettled(finalDistance) && angularSettledUtil.isSettled(angleError * 180.0 / PI)) || timeoutTimer.getDtFromMark().convert(okapi::millisecond) > timeout.convert(okapi::millisecond); // converted to degrees for ease of settle error, etc.
	}
	else
	{
		_isSettled = (distanceSettledUtil.isSettled(finalDistance) && angularSettledUtil.isSettled(angleError * 180.0 / PI));
	}
}
...
// 	double distance = sqrt(pow(endPoint.x.convert(inch) - motioncontrol::currX.convert(inch), 2) + pow(endPoint.y.convert(inch) - motioncontrol::currY.convert(inch), 2));
// 	return (distance < 2);
// }
...
void AdaptivePurePursuit::runPathAsync(path::Path *path, QTime timeO)
{
	timeout = timeO;
	timeoutTimer.placeMark();
	setPath(path);
	start();
}
...
{
	isLooping = false;
	discoChassis.stop(); // reEEEEEEEE this'll do for now, far too lazy to restructure ngl
	resetSettled();
}
